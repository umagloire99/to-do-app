...
while True:
    ...
    if option == '1':
        ...
        if task.isidentifier():  # check if your task can be identified so is not empty
            task = do.setTask(task)
            status = str(input('Enter the status of your task (Done or Not done): '))
            do.setStatus(status)
            Date = task_date = str(input("Enter the date that you will execute your task (Eg: DD/MM/YY): "))
            do.setDate(Date)
